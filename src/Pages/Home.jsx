...
const Home = () => {
  ...
  const cartItems = useSelector((state) => state.cart.cart); // ✅ fixed selector
    ...
    <div className="px-6 py-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {filteredData.map((data) => {
          const isInCart = cartItems.some((item) => item.id === data.id); // ✅ works now
          return (
            <div key={data.id} className="bg-white rounded-lg shadow hover:shadow-lg transition">
              <img
